...
const AllUsers = () => {
  ...
  const handleMakeAdmin = (user) => {
    fetch(`https://y-tigermursa.vercel.app/users/admin/${user._id}`, {
      method: "PATCH",
    })
      .then((res) => res.json())
      .then((data) => {
  ...
  const handleMakeInstructor = (user) => {
    fetch(`https://y-tigermursa.vercel.app/users/instructor/${user._id}`, {
      method: "PATCH",
    })
      .then((res) => res.json())
      .then((data) => {
  ...
  const handleDelete = (user) => {
    fetch(`https://y-tigermursa.vercel.app/users/${user._id}`, {
      method: "DELETE",
    })
      .then((res) => res.json())
      .then((data) => {
            ...
            {users.map((user, index) => (
              <tr key={user._id}>
                ...
                <td>
                  {user.role === "admin" ? (
                    <button
                      className="btn hover:bg-green-700 bg-green-700 rounded-full  text-white"
                      disabled
                    >
                      admin <FaShieldAlt></FaShieldAlt>
                    </button>
                  ) : (
                    <button
                      title="Make admin"
                      ...
                      disabled={user.role === "instructor"} // Add this line to disable the button
                    >
                      admin <FaShieldAlt></FaShieldAlt>
                    </button>
                  )}
